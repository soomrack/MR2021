...
class BPTree{
    friend class Node;
private:
    int b_factor;               //ветвистость, влияющая на кол-во ключей/детей... хотя тут могу в терминах ошибаться
...
private:                            //вспомогательные плюшки
    int H;                     //высота дерева
    bool Hplus;                 //высота изменяется когда появляется новый корень, но вот дальнейшие действия у меня жестко завязаны на текущей высоте... но изменить ее надоы
...
private://dell
    int search_place_for_del(int leaf, Node* &ref_node, int floor, int floors_for_combining);

    void combining_nodes(Node* &ref_node,Node* &ref_parent_node, int floor);
private://print
