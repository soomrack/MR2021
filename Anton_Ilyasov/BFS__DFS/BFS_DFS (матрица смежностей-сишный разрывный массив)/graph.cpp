...
Graph::Graph(int num_of_vertices, const int other_matrix[][6]) { //матрица смежности создается на основе исходной матрицы
    this->num_of_vertices = num_of_vertices;
    adjacency_matrix = new int* [num_of_vertices];
    for (int i = 0; i < num_of_vertices; i++) {
        adjacency_matrix[i] = new int[num_of_vertices];
        for (int j = 0; j < num_of_vertices; j++) {
            adjacency_matrix[i][j] = other_matrix[i][j];
        }
    }
}
...
Graph::Graph(const Graph& other_matrix) //конструктор копирования
{
    ...
    for (int i = 0; i < num_of_vertices; i++) {
        for (int j = 0; j < num_of_vertices; j++) {
            adjacency_matrix[i][j] = other_matrix.adjacency_matrix[i][j];
        }
    }
}
